...
function renderDaftarList() {
  ...
  daftarStudents.forEach((s) => {
    ...
    // Toggle expand on item click (but not when clicking detail elements)
    item.onclick = (e) => {
    ...
    if (isExpanded) {
      ...
      // FIX: removed inline onclick, added data-nama for safe special-char handling
      html += `
        <div class="daftar-detail">
          <div class="attendance-list">
            ${attendanceHtml}
          </div>
          <button class="btn-expel" data-nama="${encodeURIComponent(s.nama)}">
    ...
    // FIX: attach expel listener safely after DOM insertion
    ...
    if (expelBtn) {
      expelBtn.addEventListener('click', (e) => {
        ...
        const student = daftarStudents.find(st => st.nama === nama);
        if (student) openRemoveModal(student);
      });
    }
...
function closeRemoveModal() {
  ...
  // CRITICAL FIX: reset inline styles that block interaction
  if (removeToggleTrack) {
    removeToggleTrack.style.pointerEvents = "";
    removeToggleTrack.style.opacity = "";
  }
  if (removeConfirmBtn) {
    removeConfirmBtn.style.background = "";
  }
}
