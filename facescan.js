...
async function initFaceScan() {
  ...
  try {
    ...
    faceAlreadySubmitted.clear();
        totalStudents.forEach(s => {
      const st = (sheetStatus.get(s.id) || "").trim().toUpperCase();
      if (!st) return;

      if (currentPeriod?.isPagi && ["PAGI", "HADIR", "TERLAMBAT", "TELAT"].includes(st)) {
        faceAlreadySubmitted.add(s.id);
    ...
    // 2.5. Respect threshold & semester from Config
  try {
    const { data: cfg, error } = await sb.from('Config').select('threshold, current_semester').single();
    if (!error && cfg) {
      if (typeof cfg.threshold === 'number') {
        faceThreshold = cfg.threshold;
        console.log("Face threshold set from config:", faceThreshold);
      }
      if (cfg.current_semester) {
        currentSemester = cfg.current_semester;
      }
    }
  } catch (e) {
...
async function loadFaceDatabase() {
  const cacheKey = safeCacheKey('face_desc', currentEkstra || 'MASTER', currentSemester);

  // === SAFE CACHE ATTEMPT ===
  try {
    const cached = await safeCacheGet(cacheKey);
    if (cached && typeof cached.ts === 'number' && (Date.now() - cached.ts < 86400000)) {
      faceDescriptors = cached.data || [];
      console.log('[FaceScan] Cache hit:', faceDescriptors.length, 'descriptors');
      return;
    }
  } catch (e) {
    console.warn('[FaceScan] Cache read failed, using network:', e);
  }

  // === YOUR ORIGINAL CODE (completely untouched) ===
  const isMaster = currentEkstra === "MASTER";
  let query = sb.from('Database').select('id, nama, kelas, ekstra, face_id');
  if (!isMaster) {
    query = query.eq('ekstra', currentEkstra);
  }
  
  const { data: students, error } = await query;
  if (error) throw new Error(error.message);

  console.log("Raw students count:", students?.length || 0);
  if (students?.[0]) {
    console.log("Raw face_id type:", typeof students[0].face_id);
    console.log("Is array?", Array.isArray(students[0].face_id));
    console.log("Sample:", students[0].face_id?.slice?.(0, 3) || students[0].face_id);
  }
  ...
  for (const s of students || []) {
    ...
    if (typeof fid === 'string') {
      try { fid = JSON.parse(fid); } catch(e) { fid = null; }
    }
    if (fid && Array.isArray(fid) && fid.length === 128) {
  ...
  // === SAFE CACHE SAVE (fire-and-forget) ===
  safeCacheSet(cacheKey, { ts: Date.now(), data: faceDescriptors });
}
...
async function submitFaceChunk(opts = {}) {
  const silent = opts.silent !== false;

  if (isSendingChunk || faceUnsentQueue.length === 0) return;
  
  clearAutoSubmitTimer(); // prevent idle timer from double-firing
  isSendingChunk = true;
  ...
  const inserts = chunk.map(s => {
    let period = null;
    if (s.status === "PAGI") period = "PAGI";
    else if (s.status === "HADIR" || s.status === "TERLAMBAT") period = "EKSTRA";
  ...
  try {
    if (inserts.length > 0) {
      const { error } = await sb
        .from('Attendance')
        .upsert(inserts, { 
          onConflict: 'student_id,date,period,semester',
    ...
    chunk.forEach(s => {
      faceSyncedNames.add(s.id);
      faceAlreadySubmitted.add(s.id);
      // Intentionally keep in faceScanned so photo stays until next scan
    });
    saveFaceSession();
    clearBundle();
    updateVerifyPanel();
    ...
    if (faceUnsentQueue.length > 0) {
      ...
      submitFaceChunk(opts);
      return;
    }
  } catch (err) {
    console.error("Background send error:", err);
    if (!silent) showStatus("Koneksi bermasalah, coba lagi", "error");
  }
  ...
  faceRecognitionInterval = setInterval(async () => {
    ...
    for (const det of resizedDetections) {
      ...
      if (faceDescriptors.length === 0) {
        ...
        const id = bestMatch.id;           // use ID
        const nama = bestMatch.nama;       // keep name for display
confirmedThisFrame.add(id);

if (faceAlreadySubmitted.has(id)) {
          boxColor = "#10b981";
          setFaceRibbon(refs, "state-success", "Siswa sudah di scan", nama + " - " + bestMatch.kelas);
        }
        // ── ALREADY SCANNED THIS SESSION ──
        else if (faceScanned.has(id)) {
          boxColor = "#10b981";
          setFaceRibbon(refs, "state-success", "Siswa sudah di scan", nama + " - " + bestMatch.kelas);
        }
        // ── BUILDING CONFIDENCE (3-frame lock) ──
        else {
          const pending = facePendingConfirm.get(id); 
          let count = 1;
          if (pending) count = pending.count + 1;
          facePendingConfirm.set(id, { count, data: bestMatch });
    ...
    faceLastDetected.set(id, now);
    addFaceScan(bestMatch);
}
facePendingConfirm.delete(id);
          }
        }
      } else {
        boxColor = "#ef4444";
    ...
    // Clear pending for faces that disappeared this frame
        // Clear pending for faces that disappeared this frame
    for (const [id] of facePendingConfirm) {
      if (!confirmedThisFrame.has(id)) {
        facePendingConfirm.delete(id);
      }
    }
  }, 500);
...
function addFaceScan(student) {
  ...
  faceScanned.set(student.id, scanData);
  ...
  // Fire immediately if we hit 5, otherwise wait 30s idle
  if (faceUnsentQueue.length >= 5 && !isSendingChunk) {
    clearAutoSubmitTimer();
    submitFaceChunk({ silent: true });
  } else {
...
function updateFaceStats() {
  ...
  const sudahSheet = totalStudents.filter(s => faceAlreadySubmitted.has(s.id)).length;
const sudahScan = Array.from(faceScanned.keys()).filter(id => 
    totalStudents.some(s => s.id === id)
).length;

  // Unique "done" count — avoid double-counting if a student is both in sheet AND scanned
  const sudahSet = new Set();
  totalStudents.forEach(s => { 
    if (faceAlreadySubmitted.has(s.id)) sudahSet.add(s.id); 
  });
  faceScanned.forEach((_, id) => {
    if (totalStudents.some(s => s.id === id)) sudahSet.add(id);
  });
...
function renderFaceScannedList() {
  ...
  // Only show items still waiting to be sent
  const scans = [...faceUnsentQueue];
  ...
  list.innerHTML = scans.map(s => `
    <div class="face-scanned-item">
      ...
      <div class="face-scanned-info">
        ...
        <div class="face-scanned-meta">${escapeHtml(s.kelas)} • ${escapeHtml(s.status)} • Menunggu...</div>
      </div>
      <button class="face-scanned-remove" onclick="removeFaceScan('${escapeHtml(s.id)}')" title="Hapus">✕</button>
    </div>
  `).join("");
}

function removeFaceScan(id) {
  faceScanned.delete(id);
  faceSyncedNames.delete(id);
  faceUnsentQueue = faceUnsentQueue.filter(s => s.id !== id);
...
async function submitFaceScans() {
  ...
  if (faceUnsentQueue.length > 0) {
    await submitFaceChunk({ silent: false });
  }
...
function showFaceScanList() {
  ...
  sorted.forEach(s => {
    const isSubmitted = faceAlreadySubmitted.has(s.id);
    const isScanned = faceScanned.has(s.id);
...
function renderVerifyList() {
  ...
  container.innerHTML = scans.map(s => {
    const isSynced = faceSyncedNames.has(s.id);
    ...
    return `
      <div class="verify-item">
        ...
        <button class="verify-item-remove" data-id="${encodeURIComponent(s.id)}" title="Hapus (face mismatch)">✕</button>
      </div>
    `;
  }).join('');
}
function removeCurrentVerifyPhoto() {
    ...
    if (!nameEl || !nameEl.textContent || nameEl.textContent === '-') return;
    // Find the student by name to get their ID
    const lastScan = Array.from(faceScanned.values()).reduce((a, b) => 
        new Date(a.timestamp) > new Date(b.timestamp) ? a : b
    );
    if (lastScan && lastScan.id) removeFaceScan(lastScan.id);
}
